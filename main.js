...
// Declarar una variable llamada nota y asignar un valor numérico entre 0 y 100
let nota = Math.floor(Math.random() * 101); // Generar un número aleatorio entre 0 y 100

// Verificar si el valor de nota es mayor que 0 
if (nota >= 0) {
  // Determinar el rango de la nota y devolver el mensaje correspondiente
  function evaluarNota(nota) {
    if (nota >= 90) {
      return "El estudiante aprueba, Excelente:D.";
    } else if (nota >= 75) {
      return "El estudiante aprueba, Bien:).";
    } else if (nota >= 60) {
      return "El estudiante aprueba su calificación fue Suficiente:|.";
    } else {
      return "El estudiante no aprobó :(.";
    }
  }

  // Imprimir la nota y el mensaje en la consola
  console.log("La nota del estudiante es: ", nota);
  console.log(evaluarNota(nota)); // Salida con la evaluación de la nota
} else {
  console.log("La nota no es válida.");
}

console.log("Para generar una nueva calificación presione F5");